...
Для корректной работы всех функций убедитесь, что у вас установлены:
...
3.  Скачайте модель Llama 3 командой в терминале:
    ```bash
    ollama pull llama3
    ```
...
```bash
git clone https://github.com/guitaramust-sudo/SellerCabinet.git
cd SellerCabinet
```
...
Команда автоматически соберет образы и поднимет клиент, основной бэкенд и AI-сервис:
...
2.  **Запуск всех сервисов:** `npm start`
    *(Команда одновременно запустит фронтенд на Vite и серверные части через concurrently).*
...
5.  **Добавление категорий:** Добавить большее количество категорий, чтобы покрыть больше типов товаров.
